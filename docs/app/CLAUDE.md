...
| Type | PT Serif everywhere, the chrome (module names, rail, header, palette, dates and numbers) and what is read (item titles, prose, chat, descriptors, tags, key caps, the search bar, an Education question body) in the one face, Geist Mono for code. `fonts.css` declares all 6 vendored cuts with the `ascent-override` / `descent-override` that centre text on its capitals, so a glyph sits level with the icon beside it |
