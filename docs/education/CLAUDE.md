...
| Page | LEFT: search, chips `active` and `completed`, boxed rows with a progress bar (an active question's is the share of parts graded, a completed one's its score), no header over the queue; MIDDLE blank: `Generate` and, at the right, `+` for a topic, which opens a `Topic name` box and a `Purpose (optional)` box (Enter or `↵` adds, Esc closes), above a row per topic (difficulty, done/asked, average, recent scores, last asked as `DD-MM-YYYY`, retire); MIDDLE selected: the title with `×`, topic and tag chips, the owner's tags each with `×` and a `+ tag` box (Enter or blur adds), then the setup pinned at the top of the track while the parts scroll (definitions, folded by their label; premise; the block capped at half the viewport, scrolling inside, a fade at its foot while more sits below the cap), each part a row (letter, title, score, caret) that folds it, graded parts sitting folded until opened, an open part showing the prompt, the `Answer` box with `Submit` or `Resubmit` (Enter; Shift+Enter breaks a line) and a status word (`sending…`, `grading…`); a completed question shows each answer read-only; the owner's feedback lines; `Complete quiz` (primary, once every part is scored), `Delete` (confirmed), `Send to session`; an action that fails says so in a fixed line under the buttons. Question prose is 16px; the session pane renders the tutor's turns as markdown with LaTeX |
