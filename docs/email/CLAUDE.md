...
| Routes | `left` (chip All/Unread/Flagged/Priority over the inbox: `UNREAD`, `STARRED`, triage `high`; `limit`, 200 by default) returning `groups` of ROWs by day, `more`, and the `read_on_open` knob; `item/{id}`; `action/{archive\|trash\|read\|unread\|star\|unstar}` with `{ids}`, `{id}` or `{filter: {query, chip}}` (400 when none selects anything); and `action/sync`, declared first so `action/{verb}` cannot swallow it, which runs the scheduler's own `tasks.sync` through `run_action` on resource `gmail` (a deferred import: `tasks.py` imports this module). A chip is served by the daemon, never by filtering a page of results, so it narrows the whole mailbox. One `_where` serves the list, the bulk actions and `email_search` |
...
### The search bar only reaches the rows the page has loaded

- Kind: gap
- Where: Email requirement 2 (search bar); `app/static/core.js` (`applyTokens`), `app/static/pages/email.js` (`load`), `app/modules/email/routes.py` (`_where`, `email_fts`)
- Found: 2026-09-20, the UI port
- Status: open, not started

What happens: the shell's search bar narrows the rows already in the browser, and the list holds only the newest 200 of the open chip. A message older than that cannot be found by typing, although `email_fts` indexes the whole mailbox and `_where` can still query it. The old page sent the typed text to `left?query=`; the ported one does not, because the bar re-renders on every keystroke and the shell searches client-side for every module.

Expected: typing in the search bar finds a message anywhere in the mailbox, the way a chip already narrows all of it.

Fix: the shell decides, because every module shares the bar. Either core hands the open page the typed text after a pause and `load()` sends it on, or Email keeps a search of its own. Whichever way, the two matchers have to agree first: the daemon's index requires every word anywhere in sender, subject or snippet, while `applyTokens` requires the typed string literally, so a row the daemon found can still be dropped in the browser.
