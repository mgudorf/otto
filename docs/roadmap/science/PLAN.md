...
| `GET /api/science/item/{id:path}` | `{id, module: "science", text: <name>, kind: "ipynb" or "py", created_at: <mtime>, kernel: {state: idle or busy, executions, started_at} or null, cells: [{index, type: code or markdown, source, execution_count, running, outputs: [{kind: stream/text/html/image/error, text?, html?, png?, ename?, traceback?}]}], source: <for .py>, actions: []}` |
| `POST /api/science/action/run` `{id, index}` | `{job}`; starts the kernel if absent; queued on `kernel:<id>` |
| `POST /api/science/action/interrupt` `{id}` | `{ok}`; immediate, bypasses the queue |
| `POST /api/science/action/restart`, `shutdown` `{id}` | `{ok}`; queued on `kernel:<id>` |
| `POST /api/science/action/set_cell` `{id, index, source}`, `insert_cell` `{id, after, type}`, `delete_cell` `{id, index}` | `{id}`; queued on `kernel:<id>` so edits never interleave with a run's output write |
| `POST /api/science/action/new` `{name}` | `{id}`; creates `<name>.ipynb` in `root` with one empty code cell; resource `science` |
| `GET /api/science/events` | server-sent events `{path, index, event: started/output/done/error/kernel, output?, state?, ts}` |
...
Departures from the artboard: cell source is editable on click (the artboard is static); a `.py` MIDDLE shows the source (the artboard shows placeholder bars); the notebook header carries `Run all`, `Interrupt`, `Restart`, `Shut down` and `+ cell` actions and a `Send to session` button (the artboard header has name, kernel and `×` only); tracebacks use `#cf7b7b`.
...
| 2 Edit | click-to-edit cell (textarea, Ctrl+Enter runs, blur saves), `set_cell`/`insert_cell`/`delete_cell`/`new`, `.py` source view | Write and run a notebook end to end without VS Code |
