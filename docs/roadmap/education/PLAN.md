...
Education keeps a queue of conceptual questions on the owner's topics, grades the owner's answers in a back-and-forth with a tutor, and tunes difficulty to keep the owner in flow. Without it the owner has no place where questions are waiting each morning and no record of what they understand.

## Sources

| Source | Governs |
|---|---|
| `docs/ARCHITECTURE.md` Education, Education Constraints | what the module must do; the topics list there is blank |
...
| `docs/design/Personal Dashboard App.dc.html` rail entry, `eduGroups`, `selInfo` for `ed`, `AGENTS.edu`, home `edu` tile | icon, LEFT rows with a 40px progress bar and date groups, inspector with `Start` / `Reschedule` / `Skip`, tutor chips and placeholder, home number `due` |
...
| 10 | Knobs in `config.toml` under `[education]`: `queue_size`, `start_difficulty`, `flow_low`, `flow_high` | Boot values like `[nightly]`; nothing hidden in code |
...
| 12 | No due dates in v0: `Reschedule` is not built | A question is due until graded or skipped; scheduling can be built on top later |
...
Rejected: grading through a MIDDLE answer form with a one-shot LLM call; whole-question grading with one score list; a per-topic rolling average with a window knob; FTS5 for search; due dates and a study calendar in v0; reading `config.toml` again inside `tools.py`.
...
| `GET /api/education/blank` | `{due, topics: [{id, name, description, difficulty, asked, graded, average, last_asked}]}`, active topics only |
...
| `POST /api/education/action/add_topic` `{name, description}` | inserts at `start_difficulty`; event `added topic` |
...
| Tool | Server | Does |
|---|---|---|
| `education_topics()` | read | active topics with difficulty, asked, graded, average |
...
| `education_add_topic(name, description)` | full | inserts at `start_difficulty` |
...
Hooks: `numbers` is the open count labelled `due`; `today` is the due queue as LEFT rows (Home shows five and `+N`); `item` as above; `context` renders the active topics with difficulty and average, the started question in full with any part scores, the due count, the last five graded titles with scores, and the latest feedback lines.
...
| Table | Columns |
|---|---|
| `topics` | `id, name UNIQUE, description, difficulty CHECK 1..5, created_at, retired_at` |
...
Status is derived: open when `graded_at` and `skipped_at` are null, started when also `started_at` is set. Last asked per topic is `MAX(questions.created_at)`.
...
|---|---|---|
| 1 | schema, manifest, `[education]` config and dataclass, the `register` seam, routes and hooks, tools, `agent.md`, page, `shell.js` entry, `test_education_end_to_end`, `test_education_tool_split` | The owner adds a topic, asks the tutor for a question, answers it in the pane part by part, watches the score bar in LEFT and the topic's difficulty move, and sees `N due` on Home |
| 2 | `tasks.py generate`: queue top-up, topic rotation, no-repeat list, JSON parse, `BudgetExceeded` to `Skipped`, cursor; `test_generate_task` | Fresh questions wait each morning; Activity shows the run and its result |
...
| `MCPServer.list_tools()` | returns tool objects with `.name`, checked today | the split test |
| Test suite | 16 passed today | baseline |
...
| Item | How |
|---|---|
| Topics | ARCHITECTURE's "Topics covered should include" is blank. After phase 1, add them on the page or tell the tutor; or list them and they are entered the same way, never seeded in code |
...
| Check | Command |
|---|---|
| A scheduled LLM run completes on this machine; `llm_runs` is empty today | after the first window: `.venv/Scripts/python.exe -c "import sqlite3; print(sqlite3.connect('data/otto.db').execute('select ts, module, task, status from llm_runs').fetchall())"` |
...
1. Which topics go in first? The requirement's list is blank, and the module has nothing to ask about until at least one exists.
2. `[education]` values proposed: `queue_size = 3`, `start_difficulty = 3`, `flow_low = 60`, `flow_high = 85`. Confirm or change.
3. `Reschedule` is dropped from v0 (decision 12). Confirm.
