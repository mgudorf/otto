...
| Page | LEFT: search with `+` beside it (clears the selection; raised while the blank composer is open), boxed rows by day, `more`. MIDDLE with nothing selected: the composer alone, sending starts a conversation. MIDDLE selected: header (icon, title, tag chips, a trash icon that deletes with confirm, `×`), the transcript (user turns as raised bubbles right-aligned at 85%, model turns through `Markdown`, tool calls as `▸ tool` with the status at the right, system lines dim), text deltas appended as they arrive, the composer pinned below with a paperclip (file picker and drop), a pulsing hue dot while a turn runs and `↵`; Enter sends, Shift+Enter breaks a line; no placeholder, no counts; a draft and its pending attachments survive a selection change. RIGHT: `files`, one boxed row per file (`name · bytes`), a click opens it in a tab; an empty panel when nothing is selected |
