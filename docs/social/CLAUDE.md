...
| Dismissal | a dismissed event leaves every chip, `today`, `queue` and the agent's context, and its item offers only the url. The row stays in `social_items`, which is what stops the scout proposing the same listing again: its prompt lists the last hundred events with their status and says a dismissed one shows what to stop bringing |
| Departures | No artboard exists for this page; LEFT and MIDDLE follow Business's shape. The chips are slices rather than kinds, and there is no `All`: the shell resets every page's chip to `All` on navigation, and an unknown chip falls back to `Upcoming`, which is the useful landing slice. Group headers carry the weekday, which the other pages' `%d %b` does not, because the day of the week is what decides whether an event is worth going to |
