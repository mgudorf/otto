...
### The list stops at 200 entries and what is typed never leaves the browser

- Kind: gap
- Where: `app/static/pages/newsfeed.js` (`load`), `app/static/core.js` (the search bar, `renderMainPage`), `app/modules/newsfeed/routes.py` (`left`)
- Found: 2026-09-20, porting the page to the new shell
- Status: open

What happens: the page asks `/left` for its 200 newest entries and nothing more. The route's `query`, `chip` and `more` are served and unread: what is typed in the search bar narrows those 200 in the browser, and entry 201 and older can be neither reached nor searched. No ported page pages.

Expected: the whole feed is reachable and searchable.

Fix: the shell settles this once for every module — a place under the list for a `More` button in the page contract, and `load()` re-run as the search bar is typed into. Serving the chip alone, the way Email does, is not enough here: it would also shut the pane of an entry the chip no longer carries, and an entry that was just accepted should stay open.
