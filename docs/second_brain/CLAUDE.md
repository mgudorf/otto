...
### A chip narrows the page in hand, not the table

- Kind: gap
- Where: `app/static/pages/second_brain.js` `filter`; `app/modules/second_brain/routes.py` `left` (`CHIPS`)
- Found: 2026-09-20, the port of the page to the new shell; narrowed 09-20-2026 once typing and `More` reached the daemon
- Status: open

What happens: what is typed now goes to `second_brain_fts` and `More` reaches the older rows, but `Tasks`, `Notes` and `Links` are still applied in the browser over the page of rows in hand. `Notes` shows the notes among the newest `ui.page_size` items, not the newest notes, so a chip can look empty while the table holds plenty. The route knows only `All` and `Tasks`; the page's four chips are its own.

Expected: a chip narrows every captured item, the way the typed words and Email's chips already do.

Fix: give `CHIPS` the kinds the page offers (`Notes` is note, quote and fact together; `Links` is link), have `load` send the open chip, and drop the page's own `filter`. Counting and paging then agree, so `more` means what it says under a chip too.