...
| Page | LEFT: search, `All` and `Tasks`, boxed rows by day, a task marked by a 10px square in the hue and struck through once done; no kind is written anywhere. MIDDLE blank: the capture box (no placeholder) over a `task` chip, a `tags` box and `↵`; Enter saves, Shift+Enter breaks a line; the kind is `task` when the chip is on, `link` when the text starts with a URL, else `note`; then open suggestions with `Accept` / `Dismiss`; MIDDLE selected: inspector with tags (`+ tag` adds on Enter, click removes), `Open` for links, `Done` for tasks, `Forget` |
