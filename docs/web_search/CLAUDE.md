...
| Dismissal | a finding disagreed with leaves Home's `Today` group as well as `Review`, and its item offers only the url. The row stays in `search_findings`, which is what stops the nightly run proposing the same url again: its prompt lists the last hundred findings with their status and says a disagreed one shows what to avoid |
