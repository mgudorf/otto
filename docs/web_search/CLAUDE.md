...
| Piece | Current state |
|---|---|
| Tables | `web_search_topics(kind money\|work\|learn, text unique, created_at)`, `web_search_findings(topic_id, kind, title, url unique, summary, found_at, status open\|agreed\|disagreed, decided_at)`; a finding keeps its kind after its topic is removed |
...
| Hooks | `numbers` (open findings, `to review`), `today` (found in the local day, a disagreed one excluded), `queue` (every open finding, newest first: Home's `Review` group), `item`, `context` (topics by kind, the open queue with urls, last run) |
| Dismissal | a finding disagreed with leaves Home's `Today` group as well as `Review`, and its item offers only the url. The row stays in `web_search_findings`, which is what stops the nightly run proposing the same url again: its prompt lists the last hundred findings with their status and says a disagreed one shows what to avoid |
...
What happens: a finding is found once, decided once, and never revisited. `web_search_findings` has no follow-up date and the nightly prompt never asks for one, so a time-bound finding the owner agreed with (a filing deadline, a release, a hearing) is not searched again when its date comes.
...
Fix: `follow_up_at` and `parent_id` on `web_search_findings`, a line in the nightly prompt asking for a follow-up date, and a finding kind `follow` for the re-search; agreed findings past their date join the next prompt and Home's `Review` group.

### No search topics are entered, so the nightly search never runs

- Kind: gap
- Where: Home page requirement 2 and the Nightly Process section; `web_search_topics` table
- Found: 2026-09-10, sync-architecture
