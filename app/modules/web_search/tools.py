...
def register(read, full, store: Store, config) -> None:
    ...
    def search_topic_add(kind: str, text: str) -> dict:
        ...
        cur = store.execute("INSERT INTO search_topics(kind, text, created_at) VALUES (?, ?, ?)", (kind, text, now_iso()))
    ...
    def search_topic_remove(id: int) -> dict:
        ...
        t = store.one("SELECT * FROM search_topics WHERE id = ?", (id,))
        if t is None:
            return {"error": f"no topic {id}"}
        store.execute("DELETE FROM search_topics WHERE id = ?", (id,))
