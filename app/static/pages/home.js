...
export function Left({ app, data, fmt }) {
  ...
  return html`<div style=${{ display: 'flex', flexDirection: 'column', gap: 20 }}>
    ...
    ${data.left.groups.map((g) => html`<div key=${`${g.label}:${g.module}`} style=${{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      <${ModuleHeader} label=${g.label} count=${g.count} hue=${g.hue} icon=${g.icon} onClick=${g.page ? () => app.go(g.module) : null} />
      ...
      ${g.more > 0 && html`<div class=${g.page ? 'dim-hover' : ''} onClick=${g.page ? () => app.go(g.module) : null} style=${{ display: 'flex', alignItems: 'center', height: 32, padding: '0 12px', ...mono13, color: T.dim, cursor: g.page ? 'pointer' : 'default', borderRadius: 6 }}>+${g.more}</div>`}
    </div>`)}
  </div>`;
}
