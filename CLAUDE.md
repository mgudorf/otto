...
5. **Two faces split by job: PP Formula for the chrome, PT Serif for what is read.** The chrome is module names, the rail, the header, the palette, dates and numbers, one cut per role. The reading text is item titles, prose, chat, descriptors, tags and key caps; an Education question body is PT Serif with LaTeX rendered. Geist Mono carries code. Every cut is vendored under `app/static/vendor/fonts` and declared in `fonts.css` with the ascent and descent overrides that centre text on its capitals, so a glyph sits level with the icon beside it.
