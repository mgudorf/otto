...
5. **One face everywhere: PT Serif, chrome and reading text alike.** It carries module names, the rail, the header, the palette, dates and numbers, item titles, prose, chat, descriptors, tags and key caps, one cut per role; an Education question body is PT Serif with LaTeX rendered. Geist Mono carries code. Every cut is vendored under `app/static/vendor/fonts` and declared in `fonts.css` with the ascent and descent overrides that centre text on its capitals, so a glyph sits level with the icon beside it.
