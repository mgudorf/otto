...
description: Use when main has moved without its docs: checks docs/ARCHITECTURE.md and every docs/<module>/CLAUDE.md against the code, fixes drift, and files or closes Patches entries.
disable-model-invocation: yes
---

# Sync the docs with main

Makes `docs/ARCHITECTURE.md` (the platform) and each `docs/<module>/CLAUDE.md` (one module) describe what
...
2. For each module the log touched (`app/modules/<name>/`, `app/static/pages/<name>.js`,
   ...
   Platform changes (`app/*.py`, `app/static/` outside `pages/`, `config.toml`) go to `ARCHITECTURE.md`.
