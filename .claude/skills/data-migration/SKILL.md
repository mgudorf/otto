---
name: data-migration
description: Move or reshape rows in the live database: back it up, confirm when it is large, prove the migration on the suite and on a copy, then run it once against data/otto.db.
disable-model-invocation: yes
---

# Migrate the live data

Runs from the primary checkout (`C:\Users\gudo\Desktop\otto`): the live database `data/otto.db` and the `.venv` exist only there. The migration itself is code on `main` with a test, never a one-off script: a table rename is a line in `app/migrate.py` `RENAMES`, a reshape is `app/modules/<module>/migrate.py` behind `python -m app.modules.<module> migrate`. It is idempotent (a second run adds nothing) and it never drops a source table; dropping is a separate request I make in words. Nothing here changes a row until step 5.

## Do this

1. Size it. `data/otto.db` plus `data/otto.db-wal`, in bytes, and the free space on the drive. Say both.
2. Back it up. `Store.backup` (the SQLite backup API, which reads through the WAL while the daemon runs) into `data/backups/otto-<YYYYmmdd-HHMMSS>-pre-<slug>.db`. Open the copy and run `PRAGMA integrity_check`; anything but `ok` means stop and report. Say the path and size.
3. Confirm when it is large. Over 1 GB: stop, state the size, the backup path, what the migration reads and writes, and how long the dry run took, and wait for my yes before going on. Under 1 GB: say the size and the backup path and go on; the backup is the undo.
4. Prove it.
