...
def test_education_end_to_end(config):
    ...
    async def main():
        ...
        async with client_for(app) as c:
            ...
            assert (await c.get("/api/session/education")).json()["session"] is None
            item = (await c.get(f"/api/education/item/{qid}")).json()
            ...
            # the next submit opens a fresh session; the tutor grades through its tool and the pane shows the explanation
            r = await c.post("/api/education/action/answer", json={"id": qid, "n": 1, "answer": "because the exponential dominates"})
            assert r.status_code == 200, r.text
            await settle(app)
            s = (await c.get("/api/session/education")).json()
