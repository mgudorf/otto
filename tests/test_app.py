...
def test_feed_is_every_module_in_two_modes(config):
    ...
    listed = [row("queued", "queued", 9, "found this morning", "2026-09-08T07:00", tags=["worth-reading"]),
    ...
    async def main():
        ...
        async with client_for(app) as c:
            ...
            assert [r["id"] for r in (await c.get("/api/feed?mode=recent&tags=worth-reading")).json()["items"]] == [9]
